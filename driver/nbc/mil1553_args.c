...
static int bc_num;
module_parm_array(bcs, int, &bc_num, 0);
MODULE_PARM_DESC(bcs, "bus controller number");

static int pci_bus[MAX_DEVS];
static int pci_bus_num;
module_parm_array(pci_bus, int, &pci_bus_num, 0);
MODULE_PARM_DESC(bc, "pci bus number");

static int pci_slot[MAX_DEVS];
static int pci_slot_num;
module_parm_array(pci_slot, int, &pci_slot_num, 0);
MODULE_PARM_DESC(bc, "pci slot number");
...
static int valid_insmod_args(void)
{
	if (bc_num <= 0 || bc_num > MAX_DEVS) {
		printk(KERN_ERR PFX "too many devices, not installing\n");
		return 0;
	if (bc_num != pci_slot_num || bc_num != pci_bus_num) {
		printk(KERN_ERR PFX "bc, pci_slot and pci_bus must be"
					"the same length\n");
		return 0;
	}
	for (i = 0; i < bc_num; i++) {
		if (bcs[i] <= 0 || bcs[i] > MAX_DEVS) {
			printk(KERN_ERR PFX "invalid bus controller number %d\n",
				bcs[i]);
			return 0;
		}
	}
...
static int hunt_bc(unsigned bus, unsigned slot)
{
	int i;

	for (i = 0; i < bc_num; i++)
		if (pci_slot[i] == slot && pci_bus[i] == bus)
			return bcs[i];
	return -1;
}
...
int mil1553_install(void)
{
	...
	struct mil1553_device_s *mdev;
	struct mil1553_device_s tmpdev;
	...
	for (i=0; i<MAX_DEVS; i++) {
		...
		if (bc < 0) {
			printk(KERN_ERR PFX "unconfigured device at "
					"pci (bus,slot)=(%d,%d)\n", 
