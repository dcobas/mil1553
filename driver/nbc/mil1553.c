			...
			int   flag)
{
	...
	if (flag == BEFORE) {
		...
		if ((ionr <= mil1553FIRST) || (ionr >= mil1553LAST)) {
			printk(KERN_ERR "mil1553:Illegal ioctl:ionr:%d iosz:%d iodr:%d\n",ionr,iosz,iodr);
			return;
		}
		...
		printk(KERN_ERR "mil1553:ioctl:ionr:%d[%s] iosz:%d iodr:%d[",ionr,ioctl_names[cindx],iosz,iodr);
		if (iodr & _IOC_WRITE)
			printk("W");
		if (iodr & _IOC_READ)
			printk("R");
...
/**
 * =========================================================
 * @brief Check a BC is valid
 * @param BC number to check
 * @return Pointer to device if exists, else NULL
 */

struct mil1553_device_s *check_bc(int bc)
{
...
static int valid_insmod_args(void)
{
       int i;
		      ...
		      struct mil1553_device_s *mdev)
{
	...
	if (debug_level > 4)
		printk(KERN_ERR "mil1553:start_tx:Bc:%02d UpRtis:0x%08X\n",mdev->bc,mdev->up_rtis);
	...
	if (get_queue_size(*rp,*wp,QSZ) == 0) {
		...
		if (debug_level > 5)
			printk(KERN_ERR "mil1553:start_tx:Queue empty\n");
		return;
	}
		      ...
		      struct mil1553_tx_item_s *item_array)
{
	...
	if (client->debug_level > 2)
		printk(KERN_ERR "mil1553:send_items:item_count:%d\n",item_count);
	...
	for (i=0; i<item_count; i++) {
		...
		if (check_rti_up(mdev,rtin) == 0) {
			if (client->debug_level > 2)
				printk(KERN_ERR "mil1553:send_items:CheckRtis:Error:Bc:%02d Rti:%02d Down\n",bc,rtin);
			return -ENODEV;
		}
	}
	...
	for (i=0; i<item_count; i++) {
		...
		if (client->debug_level > 3)
			printk(KERN_ERR "mil1553:send_items:Bc:%02d Queue:Rp:%02d Wp:%02d\n",
			       bc, *rp, *wp);
	...
	for (i=0; i<wa.bcs; i++) {
		...
		if (strs[bc] >= 0) {
			...
			if (client->debug_level > 3)
				printk(KERN_ERR "mil1553:send_items:Bc:%d start_tx\n",bc);
		}
	}
	...
	do {
		...
		if (get_queue_size(*rp,*wp,QSZ) > 0) {
			...
			if (client->debug_level > 4) {
				printk(KERN_ERR "mil1553:read_queue:rp:%02d wp:%02d wc:%02d icnt:%02d bc:%02d rti:%02d\n",
				       *rp,*wp,
				       rti_interrupt->wc,
				       client->icnt,
				       rti_interrupt->bc,
				...
				printk(KERN_ERR "mil1553:read_queue:rxbuf: 4 words ...\n");
				for (i=0; i<4; i++)
		...
		if (cc == -ERESTARTSYS)
			printk(KERN_ERR "mil1553:read_queue:interrupted by signal\n");
			     ...
			     struct mil1553_device_s *mdev)
{
	...
	printk(KERN_ERR "mil1553:VID:0x%X DID:0x%X BUS:%d SLOT:%d",
	       VID_CERN,
	       DID_MIL1553,
	       mdev->pci_bus_num,
	       mdev->pci_slt_num);
	...
	sprintf(bar_name,"cbmia.%d.%d.BAR2",mdev->pci_bus_num,mdev->pci_slt_num);
	len = pci_resource_len(pcur, BAR2);
	cc = pci_request_region(pcur, BAR2, bar_name);
	if (cc) {
		pci_disable_device(pcur);
		printk(KERN_ERR "mil1553:pci_request_region:len:0x%x:%s:ERROR:%d\n",len,bar_name,cc);
		return NULL;
	...
	if (cc) {
		pci_disable_device(pcur);
		printk(KERN_ERR "mil1553:request_irq:ERROR%d\n",cc);
		return NULL;
	}

	printk(KERN_ERR "mil1553:%s OK\n",bar_name);
	return pcur;
}
...
void release_device(struct mil1553_device_s *mdev) {
	...
	printk(KERN_ERR "mil1553:BC:%d RELEASED DEVICE:OK\n",mdev->bc);
}
...
/**
 * =========================================================
 * Hunt for a BC given the PCI bus and slot numbers
 */
...
int mil1553_install(void)
{
	...
	for (i=0; i<MAX_DEVS; i++) {
		...
		if (bc > 0) {
			wa.mil1553_dev[bc] = mdev;

			wa.mil1553_dev[bc].bc = bc;
			wa.mil1553_dev[bc].in_use = 1;
			wa.mil1553_dev[bc].tx_queue = &wa.tx_queue[bc];

			spin_lock_init(&wa.mil1553_dev[bc].lock);
			spin_lock_init(&wa.mil1553_dev[bc].tx_queue->lock);

			init_device(&wa.mil1553_dev[bc]);

			printk(KERN_ERR "mil1553:BC:%d SerialNumber:0x%08X%08X\n",
				bc,wa.mil1553_dev[bc].snum_h,
				   wa.mil1553_dev[bc].snum_l);
			wa.bcs++;
		} else
			printk(KERN_ERR "mil1553:bus:%d slot:%d Not declared\n",
			       mdev.pci_bus_num, mdev.pci_slt_num);

		pdev = mdev.pdev;
	}
	printk(KERN_ERR "mil1553:Installed:%d Bus controllers\n",wa.bcs);
	return 0;
}
...
error_exit:
	...
	if ((client) && (client->debug_level > 4))
		printk(KERN_ERR "mil1553:Ioctl:%d:ErrorExit:%d\n",ionr,cc);
...
void mil1553_uninstall(void) {
	...
	printk(KERN_ERR "mil1553:Driver uninstalled\n");
}
