static char rcsid[] = "$Id: pow-v_procos.c,v 1.27 2011/06/27 15:32:52 nmn Exp $";
...
sproco(r03bav,r03bav_dtr,double)
{
	...
	value[15] = (double) dtrs->aqn;
	value[16] = (double) dtrs->aqn1;
	value[17] = (double) dtrs->aqn2;
	value[18] = (double) dtrs->aqn3;
}
...
sproco(r03bcv,r03bcv_dtr,double)
{
	...
	value[12] = (double) dtrs2->ccv;
	value[13] = (double) dtrs2->ccv1;
	value[14] = (double) dtrs2->ccv2;
	value[15] = (double) dtrs2->ccv3;
...
sproco(r03bufv,r03bufv_dtr,double)
{
	...
	value[12] = (double) ctrl_ptr->ccv;
	value[13] = (double) ctrl_ptr->ccv1;
	value[14] = (double) ctrl_ptr->ccv2;
	value[15] = (double) ctrl_ptr->ccv3;
...
sproco(r03ccvv,r03ccvv_dtr,double)
{
	...
	if ((dtr->cnnt == 0) && !((dtr->elmstr != 0) && (dtr->plsdbl != (-1)))) {
		*coco = EQP_NOTCON;
		return;
	}
	...
	switch (dtr->casesel) {
		case 0:
			*value = (double) dtrs->ccv;
			if (dtrs->ccv_change != 1)
				*coco = EQP_CCLNSET;
			return;
		case 1:
			*value = (double) dtrs->ccv1;
			if (dtrs->ccv1_change != 1)
				*coco = EQP_CCLNSET;
			return;
		case 2:
			*value = (double) dtrs->ccv2;
			if (dtrs->ccv2_change != 1)
				*coco = EQP_CCLNSET;
			return;
		case 3:
			*value = (double) dtrs->ccv3;
...
sproco(r03confv,r03confv_dtr,double)
{
	...
	value[10] = (double) conf_ptr->i_nominal;
	value[11] = (double) conf_ptr->resolution;
	value[12] = (double) conf_ptr->i_max;
	value[13] = (double) conf_ptr->i_min;
	value[14] = (double) conf_ptr->di_dt;
	value[15] = (double) conf_ptr->mode;
}
...
sproco(r03datev,r03datev_dtr,int)
{
	...
	value[0] = (int) dtrs->protocol_date.sec;
	value[1] = (int) dtrs->protocol_date.usec;
}
...
typedef struct {
	int     hwmnx;          /* RO  Hardware min/max (float !)               */
...
{
	int    property;
	float  *pointer;        /* necessary for conversion int to float */
...
sproco(r03riv,r03riv_dtr,int)
{
	...
	switch (dtr->casesel) {
	    case 1:     *value = (int) dtrs->specialist; return;
	    case 2:     *value = (int) dtrs->phys_status; return;
	    case 3:     *value = (int) dtrs->static_status; return;
	    case 4:     *value = (int) dtrs->ext_aspect; return;
	    case 5:     *value = (int) dtrs->status_qualif; return;
	    case 6:     *value = (int) dtrs->busytime; return;
	    default:    *coco = EQP_ILLDTENT; return;
	}
}
...
sproco(r03rrv,r03rrv_dtr,double)
{
	...
	/* select value corresponding to case data column */
	switch (dtr->casesel) {
		case 0: *value = (double) dtrs->aqn; return;
		case 1: *value = (double) dtrs->aqn1; return;
		case 2: *value = (double) dtrs->aqn2; return;
		case 3: *value = (double) dtrs->aqn3; return;
		default: *coco = EQP_ILLDTENT; return;
	}
}
...
sproco(w03ccsav,w03ccsav_dtr,int)
{
	...
	if (new_value == 1) {
		...
		pkt.stamp = membno;
		pkt.next = NULL;
		memcpy(&pkt.pkt[0], &dtr->ccac[0], 24);
		memcpy(&pkt.pkt[12], &da, 8);             /* set TOD field */
		pkt.pktcnt = 44;                         /* size of control message */
...
typedef struct {
 int      hwmnx           ; /* WO  Hardware min/max (float !)               */
