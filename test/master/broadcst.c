...
int broadcst()
{
	...
	do  {
	    ...
	    if (XILINX==0) continue; /* pas de last command pour les vieilles cartes */
	...
	clr_line();
	if (erreur) print(0,"Broadcast data a donne %d erreurs\n",erreur);
	...
	clr_line();
	if (erreur) print(0,"Broadcast mode code a donne %d erreurs\n",erreur);
	return(erreur);
}
