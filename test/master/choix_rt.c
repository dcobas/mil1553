...
/***************** choise of BC ************************************/
	if (get_connected_bc(&mask)) {BAD("Probleme avec get_connected_bc");return(0);}
	printf("BC(s) Connectes :  (Mask ->%04X)\n",mask);
	n=0;
	for(i=0;i<32;i++)
	    if (mask&(1<<i)) {
		printf("  BC number: %d.\n",i);
...
/****************** choise of RT ***********************************/
	...
	if (XILINX) print(0,"It's card XILINX type\n");
	else	    print(0,"It's card not_XILINX type\n");
	return(0);
}
