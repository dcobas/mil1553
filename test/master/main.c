...
extern int choix_rt(),mode_code(),mode_data(),broadcst(),tst_csr();
extern int rx_buf(),tx_buf(),rd_csr(),wr_csr(),cl_csr(),reset();
extern int bit_test(),bit_local(),bounce(),collision(),signature(),rt30();
extern int tst_buffer(), full_data();
...
struct mdr_tests { int index; int (*fn)(); char txt[80]; };

int config(),seq_tst(),exe_seq(),accept(),accept_pz(),accept_cr();
int comm_tst(),comm_exe(),not_test();
...
static struct {int indx,iteration;} seq[99];
...
#if 0
void *inter(sig,code,scp)                     /* signal handling */
int sig, code;
...
{
	char *ttyname();
	if ((f_in=open(ttyname(0),2))<0)        {perror("open");exit(0);}
	if (ioctl(f_in,TCGETA,&old)<0)          {perror("ioctl");exit(0);}
	...
	if (ioctl(f_in,TCSETA,&new)<0)          {perror("ioctlset");exit(0);}
}
...
void epilogue()
{
	...
	old.c_cc[VQUIT]=3;
	old.c_cc[VKILL]=4;
	if (ioctl(f_in,TCSETA,&old)<0)          {perror("ioctlset");exit(0);}
	close(f_in);
}
...
int seq_tst()
{
	...
	do {
	    ...
	    scanf("%s",choix);
	    if (choix[0]=='0') {seq[i].indx=0;break;}
	    if (menu != 0)
		seq[i].indx=(choix[0]-'a')+1+20*(2*menu-1);
		else seq[i].indx=(choix[0]-'a')+1;
...
int exe_seq()
{
	...
	do {
		...
		if (go_on==0) return(er); /* ^C stops the current loop */
		if ((stop_after_err == 1) && (er != 0)) return(er); /* we stop at the first error in accept */
		i++;	/* next step in the sequence */
		}
	    if (t_err && er) return(er); /* On s'arrette quand il y a des erreurs */
	    } while (!(stop_after_err));
	return(er);
}

int accept_cr()     /* test standard d'acceptance d'une carte */
{
	int er, cer;
	char Tx_CR[] = "Cherche volontaire pour ....\n";

	bouclage_seq=0;
	seq[ 0].indx=104;seq[ 0].iteration=1;	/* reset du bc/rt	*/
	seq[ 1].indx=101;seq[ 1].iteration=1;	/* test mode code	*/
	seq[ 2].indx=102;seq[ 2].iteration=1;	/* test mode donnee	*/
	seq[ 3].indx=103;seq[ 3].iteration=1;	/* test broadcast	*/
	seq[ 4].indx=105;seq[ 4].iteration=1;	/* test des signatures	*/
	seq[ 5].indx=106;seq[ 5].iteration=1;	/* test du rt30		*/
	seq[ 6].indx=104;seq[ 6].iteration=1;	/* reset du bc/rt	*/
	seq[ 7].indx=20;seq[ 7].iteration=1;	/* new_line if needs	*/
...
int accept_pz()     /* test standard d'acceptance d'une carte */
{
	int er;

	bouclage_seq=0;
	seq[ 0].indx=71;seq[ 0].iteration=1;	/* test des signatures	*/
	seq[ 1].indx=72;seq[ 1].iteration=1;	/* test du rt30		*/
	seq[ 2].indx=73;seq[ 2].iteration=1;	/* reset du bc/rt	*/
	seq[ 3].indx=61;seq[ 3].iteration=1;	/* test mode code	*/
	seq[ 4].indx=62;seq[ 4].iteration=1;	/* test mode donnee	*/
	seq[ 5].indx=63;seq[ 5].iteration=1;	/* test broadcast	*/
	seq[ 6].indx=67;seq[ 6].iteration=1;	/* init du mode TEST	*/
	seq[ 7].indx=64;seq[ 7].iteration=1;	/* test du csr		*/
	seq[ 8].indx=65;seq[ 8].iteration=1;	/* test du RxBuffer	*/
	seq[ 9].indx=66;seq[ 9].iteration=1;	/* test du TxBuffer	*/
	seq[10].indx=20;seq[10].iteration=1;	/* new_line if needs	*/
...
int accept()        /* test standard d'acceptance d'une carte */
{
	int er;

	bouclage_seq=0;
	seq[ 0].indx=31;seq[ 0].iteration=1;	/* test des signatures	*/
	seq[ 1].indx=32;seq[ 1].iteration=1;	/* test du rt30		*/
	seq[ 2].indx= 2;seq[ 2].iteration=1;	/* reset du bc/rt	*/
	seq[ 3].indx=21;seq[ 3].iteration=1;	/* test mode code	*/
	seq[ 4].indx=22;seq[ 4].iteration=1;	/* test mode donnee	*/
	seq[ 5].indx=23;seq[ 5].iteration=1;	/* test broadcast	*/
	seq[ 6].indx=27;seq[ 6].iteration=1;	/* init du mode TEST	*/
	seq[ 7].indx=24;seq[ 7].iteration=1;	/* test du csr		*/
	seq[ 8].indx=25;seq[ 8].iteration=1;	/* test du RxBuffer	*/
	seq[ 9].indx=26;seq[ 9].iteration=1;	/* test du TxBuffer	*/
	seq[10].indx= 2;seq[10].iteration=1;	/* reset du bc/rt	*/
	seq[11].indx=28;seq[11].iteration=1;	/* test LOCAL		*/
	seq[12].indx=29;seq[12].iteration=1;	/* test remote bounce	*/
	seq[13].indx=30;seq[13].iteration=1;	/* test remote collision*/
	seq[14].indx= 0;
	accept_flg=1;
	er=exe_seq();
	accept_flg=0;
...
/********************************************************/
int exec(indx,tr)
int indx,tr;	/* index a executer + nombre de tours a effectuer */
{
	...
	while (tb_pt->index!=-1) {
		...
		do {
			...
			if (erreur == -1) 
				...
				convbin ((unsigned short *)erreur, st);
				erreur = 0;
				print (1,"\t\t\t    MC(0) - Status: %s",st);
				}
			} 
		    er+=erreur;
		    if (indx==1) break;	/* choix_rt ne doit pas boucler! */
		    if (indx==3) break;	/* config ne doit pas boucler! */
		    if ((indx==4) || (indx==13) || (indx==38) || (indx==78) || (indx==118))break;	/* seq_tst ne doit pas boucler! */
		    clr_line();
		    if ((tt) || (tr>1) || (tr==0)) {
			if ((indx == 14) || (indx == 39) || (indx == 79) || (indx == 119))
			    sprintf(st,"    TOUR [%d]",++tt);
			    else sprintf(st,"\tTOUR [%d]",++tt);
			} else sprintf(st,"\t");
		    if (indx != 14) {
			if (erreur) 
			    print(0,"%s%s  %d errors in\n",st,tb_pt->txt,erreur);
			    else {
				if (indx!=30&&indx!=70&&indx!=25&&indx!=26&&indx!=65&&indx!=66&&indx!=33&&indx!=73&&indx!=2&&indx!=104&&indx!=34&&indx!=35&&indx!=36&&indx!=74&&indx!=75&&indx!=76) 
				    print(1,"%s OK",st);
				    else if (indx==25||indx==26||indx==65||indx!=66)
					print (2,"\n");
				}
			} 
		    ...
		    if (go_on==0) {print(0," Interrupted by your request !(^e)\n");break;} /* ^C arrete l'execution */
		    if (tt && (indx==5 || indx==44)) print(0,"\n");	/* c'est plus clair */
		    if ((stop_after_err == 1) && (er != 0 )) break;
		    } while ((tr==0) || (--tr>0));
		return(er);
		}
	    tb_pt++;
...
/*	int sig;	*/
	...
	if ((f_in=open(ttyname(0),2))<0)        {perror("open");exit(0);}
	if (ioctl(f_in,TCGETA,&iniold)<0)       {perror("ioctl");exit(0);}
	iniold.c_cc[VINTR]=5;
	iniold.c_cc[VQUIT]=3;
	iniold.c_cc[VKILL]=4;
	if (ioctl(f_in,TCSETA,&iniold)<0)       {perror("ioctlset");exit(0);}
	close (f_in);
	...
	signal(SIGINT,inter);
#endif
	iteration=1;t_err=1;/*log_print=2;*/
	setbuf(stdout,0);
}

void ter_exit(int cc)
{
	if ((f_in=open(ttyname(0),2))<0)  {perror("open");exit(0);}
	if (ioctl(f_in,TCSETA,&iniold)<0) {perror("ioctl");exit(0);}
...
int main()
{
	...
	init();
	printf("\n\n\n   W E L C O M E    T O    T H E    B A N C   O F   T H E   T E S T   1 5 5 3\n\n");
	logfile();
