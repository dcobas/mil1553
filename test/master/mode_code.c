...
int mode_code()
{
	...
	do {
		...
		if (TR==0) {
			...
			SA=31;
			TR=0;
			} else break;		/* C'est bon, on a tout fait...*/
		}
	    if (mc==18) continue;			     /* pas de MC18 dans le test */
	    if ((TR==1) && ((mc==2) || (mc==8))) continue;   /* pas de MC2 ou 8 en read */
/*	    if ((XILINX==0) && (TR==0) && (mc<16)) continue; /* pas de MC0-15 en write pour les anciennes cartes */
	    if ((TR==0) && (mc<16)) continue;
